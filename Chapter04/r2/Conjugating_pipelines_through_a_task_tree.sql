--run the aggregation query used by the task tree
SELECT
    C.C_NAME,
    SUM(L_EXTENDEDPRICE),
    SUM(L_TAX)
...
--create a database and target table
...
--create the root task that clears the table
USE DATABASE TASK_DEMO;
...
--make the insert task run after the clear task
ALTER TASK INSERT_ORDERING_CUSTOMERS
ADD AFTER CLEAR_ORDERING_CUSTOMERS;

--describe the child task to verify the predecessor
